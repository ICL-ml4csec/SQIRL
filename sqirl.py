...
def main(stdscr):
    ...
    if len(sys.argv) < 2:
        ...
        parser.add_option('--training',
                            ...
                            help="Set agent to training conditions", default=False)
        ...
        for current_episode in range(no_episodes):
            ...
            if current_episode == 0:
                ...
                if env.current_input_entry.input.action in seen_pairs:
                    if [param['name'] for param in env.current_input_entry.input.inputs if param['input_use'] == 'injection_point'][0] in seen_pairs[env.current_input_entry.input.action]:
                        continue
                    seen_pairs[env.current_input_entry.input.action].append([param['name'] for param in env.current_input_entry.input.inputs if param['input_use'] == 'injection_point'][0])
                else:
                    print(f"Testing: {[param['name'] for param in env.current_input_entry.input.inputs if param['input_use'] == 'injection_point'][0]}, {env.current_input_entry.input.action}")
            ...
            while(not game_done and not max_timestamp_reached):
                ...
                all_actions[str([selected_action["action"],selected_action["range"][0],selected_action["type"]])][current_time_stamp] += 1
                
                # add q_value
                if q_value is not None:
                    q_value_all_actions[str([selected_action["action"],selected_action["range"][0],selected_action["type"]])][current_time_stamp] += q_value
                ...
                if is_learning:
                    stdscr.addstr(f"Running episode: {current_episode+1}, average time taken: {round(avg_time_taken,2)} sec, total number of wins: {total_no_wins}, total number of losses: {total_no_losses}\n")
                else:
                    stdscr.addstr(f"Running episode: {current_episode + 1}, average time taken: {round(avg_time_taken, 2)} sec, total number of wins: {total_no_wins}, total number of losses: {total_no_losses}\n")
                stdscr.addstr(" \n")
                if last_injection_win:
                    stdscr.addstr("Last Successfull Injection:\n")
                    stdscr.addstr(f"{last_injection_win}\n")
                    stdscr.addstr(" \n")
                ...
                stdscr.addstr("Current Run Details:\n")
                stdscr.addstr(f"Input parameter: {[param['name'] for param in env.current_input_entry.input.inputs if param['input_use'] == 'injection_point'][0]}\n")
                stdscr.addstr(f"URL of input: {env.current_input_entry.input.action}\n")
                stdscr.addstr(" \n")

                stdscr.addstr(f"current timestamp: {current_time_stamp}\n")
                stdscr.addstr(" \n")

                stdscr.addstr(f"current payload: {current_state['payload']}\n")
                stdscr.addstr(" \n")

                sql_text = current_state['sql'] if not current_state['error'] else "No SQL Executed"
                stdscr.addstr(f"current SQL: {sql_text}\n")
                stdscr.addstr(
                "  .-.-.   .-.-.   .-.-.   .-.-.   .-.-.   .-.-.   .-.-.   .-.-\n"+
                "/ / \\ \\ / / \\ \\ / / \\ \\ / / \\ \\ / / \\ \\ / / \\ \\ / / \\ \\ / / \\\n"+
                "`-'   `-`-'   `-`-'   `-`-'   `-`-'   `-`-'   `-`-'   `-`-'\n"
